...
def createMobiusFiles(extractedCSV, attachments):
    ...
    df.to_csv(attachments[0]['file'] + "_" + str(date) + ".zip", index=False, header=False,
              compression=compression_opts)

    df = df['Test Number']
    compression_opts = dict(method='zip', archive_name=attachments[1]['file'] + '.csv')
    df.to_csv(attachments[1]['file'] + "_" + str(date) + ".zip", index=False, header=False,
              compression=compression_opts)

    filesToAttach = [attachments[0]['file'] + "_" + str(date) + ".zip",
                     attachments[1]['file'] + "_" + str(date) + ".zip"]
...
def archiveAllFiles(archiveDir, attachments, extractedFile):
    for files in attachments:
        shutil.move(files, archiveDir)
    shutil.move(extractedFile, archiveDir)
